...
const metrics = [
  ...
  },
  {
    ...
    decimals: 0,
    label: "CI workflows",
    sublabel: "fuzzing · mutation testing · chaos · sanitizers",
  },
  {
    value: 9.5,
    suffix: "/10",
    ...
    sublabel: "supply-chain security rating",
  },
];

function AnimatedNumber({ end, suffix = "", prefix = "", decimals = 0 }: { end: number; suffix?: string; prefix?: string; decimals?: number }) {
        ...
        {displayValue.split("").map((char, i) => (
          <span
            ...
            className={`inline-block transition-all duration-150 ${
              isScrambling && char !== "," && char !== "." ? "blur-[1px]" : ""
            }`}
          >
            {char}
          </span>
          ...
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-sm font-mono text-muted-foreground">
                every number below traces to this repo
              </span>
            </div>
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[140px] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
              ...
              Numbers you
              <br />
              <span className="text-muted-foreground">can check.</span>
            </h2>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-10 lg:p-14 transition-all duration-700 ${
            ...
            <div className="text-4xl md:text-5xl lg:text-6xl font-display tracking-tight mb-4 whitespace-nowrap overflow-hidden">
              <AnimatedNumber end={metrics[0].value} suffix={metrics[0].suffix} prefix={metrics[0].prefix} decimals={metrics[0].decimals} />
            </div>
            <div className="mb-6">
          ...
          {metrics.slice(1).map((metric, index) => (
            ...
            >
              ...
              <div className="text-3xl md:text-4xl lg:text-5xl font-display tracking-tight w-full">
                <AnimatedNumber end={metric.value} suffix={metric.suffix} prefix={metric.prefix} decimals={metric.decimals} />
              </div>
            </div>
          ))}
        </div>
